...
using create_input_layout_fn = HRESULT(STDMETHODCALLTYPE*)(ID3D11Device*,
                                                           ...
                                                           const void*, SIZE_T,
                                                           ID3D11InputLayout**);
using create_vertex_shader_fn = HRESULT(STDMETHODCALLTYPE*)(ID3D11Device*, const void*, SIZE_T,
                                                            ID3D11ClassLinkage*,
                                                            ID3D11VertexShader**);
using create_pixel_shader_fn = HRESULT(STDMETHODCALLTYPE*)(ID3D11Device*, const void*, SIZE_T,
                                                           ID3D11ClassLinkage*,
                                                           ID3D11PixelShader**);
                                                  ...
                                                  ID3D11Texture2D** out)
{
    ...
    ++self.created;
    // Every texture, not only the ones the dump filter retains: what a texture is for is the
    // caller's question, and the filter above exists to serve dumping rather than identification.
    if (self.options.on_texture) {
        self.options.on_texture(self.options.on_texture_user, *out, desc->Width, desc->Height,
                                static_cast<uint32_t>(desc->Format), desc->MipLevels,
...
HRESULT STDMETHODCALLTYPE hooked_create_input_layout(ID3D11Device* device,
                                                     const D3D11_INPUT_ELEMENT_DESC* descs,
                                                     UINT count, const void* signature,
                                                     SIZE_T signature_bytes,
                                                     ID3D11InputLayout** out)
{
    Observer& self = observer();
    const HRESULT result =
        self.original_create_input_layout(device, descs, count, signature, signature_bytes, out);
    if (FAILED(result) || !descs || !out || !*out) {
        return result;
    }

    // Translated here rather than handed over raw, so the callback and its game-side rule never
    // need d3d11.h. Copied onto the stack because the descriptors belong to the caller.
    rsf_observer_layout_element elements[RSF_OBSERVER_MAX_LAYOUT_ELEMENTS];
    const uint32_t copied =
        count < RSF_OBSERVER_MAX_LAYOUT_ELEMENTS ? count : RSF_OBSERVER_MAX_LAYOUT_ELEMENTS;
    for (uint32_t index = 0; index < copied; ++index) {
        elements[index].semantic_index = descs[index].SemanticIndex;
        elements[index].format = static_cast<uint32_t>(descs[index].Format);
        elements[index].input_slot = descs[index].InputSlot;
        elements[index].byte_offset = descs[index].AlignedByteOffset;
        elements[index].per_instance =
            descs[index].InputSlotClass == D3D11_INPUT_PER_INSTANCE_DATA ? 1u : 0u;
    }

    std::lock_guard<std::mutex> lock(self.guard);
    if (self.options.on_layout) {
        self.options.on_layout(self.options.on_layout_user, *out, elements, copied, count);
    }
    return result;
}

HRESULT STDMETHODCALLTYPE hooked_create_vertex_shader(ID3D11Device* device, const void* bytecode,
                                                      SIZE_T bytes, ID3D11ClassLinkage* linkage,
                                                      ID3D11VertexShader** out)
{
    Observer& self = observer();
    const HRESULT result =
        self.original_create_vertex_shader(device, bytecode, bytes, linkage, out);
    if (FAILED(result) || !out || !*out) {
        return result;
    }
    std::lock_guard<std::mutex> lock(self.guard);
    if (self.options.on_shader) {
        self.options.on_shader(self.options.on_shader_user, *out, RSF_OBSERVER_STAGE_VERTEX,
                               bytecode, static_cast<uint32_t>(bytes));
    }
    return result;
}

HRESULT STDMETHODCALLTYPE hooked_create_pixel_shader(ID3D11Device* device, const void* bytecode,
                                                     SIZE_T bytes, ID3D11ClassLinkage* linkage,
                                                     ID3D11PixelShader** out)
{
    Observer& self = observer();
    const HRESULT result = self.original_create_pixel_shader(device, bytecode, bytes, linkage, out);
    if (FAILED(result) || !out || !*out) {
        return result;
    }
    std::lock_guard<std::mutex> lock(self.guard);
    if (self.options.on_shader) {
        self.options.on_shader(self.options.on_shader_user, *out, RSF_OBSERVER_STAGE_PIXEL,
                               bytecode, static_cast<uint32_t>(bytes));
    }
    return result;
}
...
extern "C" rsf_observer_result rsf_observer_install(const rsf_observer_options* options)
{
    ...
    if (self.options.on_layout) {
        patch_slot(self.device_vtable, slot_create_input_layout,
                   ...
                   reinterpret_cast<void**>(&self.original_create_input_layout));
    }
    if (self.options.on_shader) {
        patch_slot(self.device_vtable, slot_create_vertex_shader,
                   reinterpret_cast<void*>(&hooked_create_vertex_shader),
                   reinterpret_cast<void**>(&self.original_create_vertex_shader));
...
extern "C" rsf_observer_result rsf_observer_uninstall(void)
{
    ...
    if (self.original_create_input_layout) {
        patch_slot(self.device_vtable, slot_create_input_layout,
                   reinterpret_cast<void*>(self.original_create_input_layout), nullptr);
        self.original_create_input_layout = nullptr;
    }
    if (self.original_create_vertex_shader) {
        patch_slot(self.device_vtable, slot_create_vertex_shader,
                   reinterpret_cast<void*>(self.original_create_vertex_shader), nullptr);
        self.original_create_vertex_shader = nullptr;
    }
    if (self.original_create_pixel_shader) {
        patch_slot(self.device_vtable, slot_create_pixel_shader,
