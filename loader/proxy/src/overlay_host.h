...
/* Load the panel, build its renderer against the game's device, and subclass the window the swap
   ...
   `device` is an `ID3D11Device*` and `swapchain` an `IDXGISwapChain*`. Returns non-zero when the
...
int rsf_overlay_host_start(void* device, void* swapchain, rsf_overlay_host_log_fn log,
                           void* log_user);
...
/* Lay out and draw one frame, from inside the Present hook and on the render thread.
   ...
   `context` is an `ID3D11DeviceContext*` and must be the immediate one. `stats` is what the panel
...
int rsf_overlay_host_present(void* context, void* swapchain, const rsf_overlay_stats* stats,
